...
class Turbo_Guard_Live_Traffic {
	...
	public static function get_traffic_paged( $per_page = 50, $offset = 0, $filter = 'all' ) {
		...
		$query = $wpdb->prepare(
			"SELECT * FROM {$wpdb->prefix}turbo_guard_traffic {$where} ORDER BY id DESC LIMIT %d OFFSET %d", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- $where is a fixed allowlisted literal from build_where(); no user input.
			absint( $per_page ),
			absint( $offset )
		);

		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared -- Custom turbo_guard_traffic table; $query is prepared above.
	...
	public static function get_traffic_count( $filter = 'all' ) {
		...
		$query = $wpdb->prepare(
			"SELECT COUNT(*) FROM {$wpdb->prefix}turbo_guard_traffic {$where}", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- $where is a fixed allowlisted literal from build_where(); no user input.
		);
