...
class Turbo_Guard_Notices {
	...
	private function __construct() {
		...
		// AJAX: flush notification cache (retained as a no-op for compatibility).
	...
	/**
	 * AJAX handler: flush the notification cache for the current site.
	 *
	 * No-op: there is no remote cache to flush. Retained so existing admin
	 * buttons keep receiving a valid JSON response.
	 *
	 * @since 1.3.0
	 */
	public function ajax_flush_notices() {
		...
		wp_send_json_success( array( 'message' => 'No remote notification cache to flush.' ) );
	}
}
