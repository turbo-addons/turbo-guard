...
<div class="wrap turbo-guard-seo-spam">
	...
	<div class="turbo-guard-page-header" style="background:linear-gradient(135deg,#7c2d12 0%,#c2410c 60%,#ea580c 100%);">
		<div class="turbo-guard-page-header-left">
			...
			<div>
				<h1><?php esc_html_e( 'SEO Spam Detector', 'turbo-guard-security-malware-scanner' ); ?></h1>
				<p><?php esc_html_e( 'Find Japanese/Chinese spam pages on your site — no Google setup required', 'turbo-guard-security-malware-scanner' ); ?></p>
			</div>
		</div>
	...
	<div class="turbo-guard-card" style="border-left:4px solid #ea580c;">
		<h2 style="color:#c2410c;"><?php esc_html_e( 'How This Works (No Google Login Needed!)', 'turbo-guard-security-malware-scanner' ); ?></h2>
		<div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;">
			<div>
				<p style="font-size:13px;color:#374151;line-height:1.6;margin:0 0 8px;">
					<?php esc_html_e( 'When hackers inject Japanese/Chinese SEO spam they leave traces on your server BEFORE Google indexes them. Turbo Guard scans for these traces locally:', 'turbo-guard-security-malware-scanner' ); ?>
				</p>
				<ul style="font-size:13px;color:#374151;margin:0 0 0 18px;line-height:1.8;">
					<li><?php esc_html_e( 'Posts/pages with Japanese or Chinese titles in your database', 'turbo-guard-security-malware-scanner' ); ?></li>
					<li><?php esc_html_e( 'PHP files in uploads folder containing CJK text', 'turbo-guard-security-malware-scanner' ); ?></li>
					<li><?php esc_html_e( '.htaccess redirect rules pointing to spam domains', 'turbo-guard-security-malware-scanner' ); ?></li>
					<li><?php esc_html_e( 'Contaminated WordPress options (siteurl, blogname)', 'turbo-guard-security-malware-scanner' ); ?></li>
				</ul>
			</div>
			<div style="background:#fff7ed;border-radius:8px;padding:14px 16px;">
				<strong style="font-size:13px;color:#92400e;display:block;margin-bottom:8px;">
					<?php esc_html_e( 'Already indexed in Google?', 'turbo-guard-security-malware-scanner' ); ?>
				</strong>
				<p style="font-size:12px;color:#78350f;margin:0 0 10px;line-height:1.5;">
					<?php esc_html_e( 'If spam pages are already in Google, connect Google Search Console in GSC Cleanup to remove them. First — clean the source files here.', 'turbo-guard-security-malware-scanner' ); ?>
				</p>
				<a href="<?php echo esc_url( 'https://www.google.com/search?q=site:' . rawurlencode( $tg_domain ) ); ?>"
					...
					printf(
						...
						esc_html__( 'Check site:%s on Google', 'turbo-guard-security-malware-scanner' ),
						esc_html( $tg_domain )
					);
					?>
				</a>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=turbo-guard-gsc' ) ); ?>" class="button button-small">
					<?php esc_html_e( 'GSC Cleanup', 'turbo-guard-security-malware-scanner' ); ?>
				</a>
			</div>
		</div>
	</div>
	...
	<div class="turbo-guard-card">
		<div class="turbo-guard-scan-hero">
			<div class="turbo-guard-scan-intro">
				<h2><?php esc_html_e( 'Scan for SEO Spam', 'turbo-guard-security-malware-scanner' ); ?></h2>
				<p><?php esc_html_e( 'Scans your database, uploads folder, and .htaccess for Japanese/Chinese spam content. Instant — no external connection needed.', 'turbo-guard-security-malware-scanner' ); ?></p>
			</div>
			<button id="turbo-guard-run-seo-scan" class="button button-primary button-large">
				<span class="dashicons dashicons-search"></span>
				<?php esc_html_e( 'Scan for SEO Spam', 'turbo-guard-security-malware-scanner' ); ?>
			</button>
		</div>
		<div id="turbo-guard-seo-scanning" style="display:none;margin-top:14px;padding-top:14px;border-top:1px solid #f3f4f6;">
			...
			<strong><?php esc_html_e( 'Scanning database, uploads, and .htaccess...', 'turbo-guard-security-malware-scanner' ); ?></strong>
		</div>
	...
	<?php if ( $tg_seo_results ) : ?>
		...
		<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:16px;">
			<?php
			$tg_seo_cards = array(
				array( 'label' => __( 'Total Found', 'turbo-guard-security-malware-scanner' ),      'value' => $tg_seo_total,                                        'sub' => __( 'spam indicators', 'turbo-guard-security-malware-scanner' ) ),
				array( 'label' => __( 'Spam Posts', 'turbo-guard-security-malware-scanner' ),       'value' => count( $tg_seo_results['spam_posts'] ),               'sub' => __( 'in database', 'turbo-guard-security-malware-scanner' ) ),
				array( 'label' => __( 'Spam Files', 'turbo-guard-security-malware-scanner' ),       'value' => count( $tg_seo_results['spam_files'] ),               'sub' => __( 'on disk', 'turbo-guard-security-malware-scanner' ) ),
				array( 'label' => __( '.htaccess Hacks', 'turbo-guard-security-malware-scanner' ),  'value' => count( $tg_seo_results['htaccess_hacks'] ),           'sub' => __( 'redirect rules', 'turbo-guard-security-malware-scanner' ) ),
			);
		...
		<?php if ( 0 === $tg_seo_total ) : ?>
			<div class="turbo-guard-card">
				<div style="text-align:center;padding:40px 20px;">
					...
					<strong style="font-size:16px;color:#14532d;"><?php esc_html_e( 'No SEO spam found on this site!', 'turbo-guard-security-malware-scanner' ); ?></strong>
					<p style="color:#6b7280;margin-top:8px;font-size:13px;">
						<?php
						printf(
							/* translators: %s: Google site search URL */
							esc_html__( 'Tip: Also check Google manually: %s', 'turbo-guard-security-malware-scanner' ),
		...
		<div class="turbo-guard-card">
			<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:10px;">
				<h2 style="margin:0;color:#dc2626;">
					...
					printf(
						...
						esc_html__( '%d Spam Posts/Pages Found in Database', 'turbo-guard-security-malware-scanner' ),
				...
				<button id="turbo-guard-delete-all-spam-posts" class="button"
					...
					printf(
						...
						esc_html__( 'Delete All %d Spam Posts (Free)', 'turbo-guard-security-malware-scanner' ),
						count( $tg_seo_results['spam_posts'] )
					);
					?>
				</button>
			...
			<p style="font-size:13px;color:#6b7280;margin-bottom:14px;">
				<?php esc_html_e( 'These posts contain Japanese or Chinese spam content. Delete them here, then use GSC Cleanup to remove them from Google index.', 'turbo-guard-security-malware-scanner' ); ?>
			</p>
			<table class="widefat turbo-guard-results-table">
				<thead>
					<tr>
						<th><?php esc_html_e( 'Title', 'turbo-guard-security-malware-scanner' ); ?></th>
						<th style="width:80px;"><?php esc_html_e( 'Type', 'turbo-guard-security-malware-scanner' ); ?></th>
						<th style="width:80px;"><?php esc_html_e( 'Status', 'turbo-guard-security-malware-scanner' ); ?></th>
						<th><?php esc_html_e( 'Reason', 'turbo-guard-security-malware-scanner' ); ?></th>
						<th style="width:160px;"><?php esc_html_e( 'Actions', 'turbo-guard-security-malware-scanner' ); ?></th>
					</tr>
				</thead>
				<tbody>
					...
					<tr>
						<td>
							<strong style="font-size:13px;"><?php echo esc_html( $tg_spam_post['title'] ? $tg_spam_post['title'] : __( '(no title)', 'turbo-guard-security-malware-scanner' ) ); ?></strong>
						...
						<td>
							...
							<a href="<?php echo esc_url( $tg_spam_post['edit_url'] ); ?>" class="button button-small" style="margin-left:4px;">
								<?php esc_html_e( 'Edit', 'turbo-guard-security-malware-scanner' ); ?>
							</a>
							<?php endif; ?>
						</td>
					</tr>
		...
		<div class="turbo-guard-card" style="border-left:4px solid #dc2626;">
			<h2 style="color:#dc2626;">
				...
				printf(
					...
					esc_html__( '%d Suspicious .htaccess Rules', 'turbo-guard-security-malware-scanner' ),
					count( $tg_seo_results['htaccess_hacks'] )
				);
				?>
			</h2>
			<p style="font-size:13px;color:#6b7280;margin-bottom:14px;">
				<?php esc_html_e( 'These .htaccess rules look suspicious. Review carefully and remove any you did not add.', 'turbo-guard-security-malware-scanner' ); ?>
			</p>
			<table class="widefat">
				<thead>
					<tr>
						<th style="width:80px;"><?php esc_html_e( 'Line', 'turbo-guard-security-malware-scanner' ); ?></th>
						<th><?php esc_html_e( 'Rule', 'turbo-guard-security-malware-scanner' ); ?></th>
						<th><?php esc_html_e( 'Reason', 'turbo-guard-security-malware-scanner' ); ?></th>
					</tr>
				</thead>
				<tbody>
		...
		<div class="turbo-guard-card">
			<h2 style="color:#dc2626;">
				...
				printf(
					...
					esc_html__( '%d Spam Files Found on Disk', 'turbo-guard-security-malware-scanner' ),
					count( $tg_seo_results['spam_files'] )
				);
				?>
			</h2>
			<p style="font-size:13px;color:#6b7280;margin-bottom:14px;">
				<?php esc_html_e( 'Go to Turbo Guard Scanner to select and delete these files in bulk. A backup ZIP is created automatically.', 'turbo-guard-security-malware-scanner' ); ?>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=turbo-guard-scanner' ) ); ?>" class="button button-small" style="margin-left:8px;">
					<?php esc_html_e( 'Go to Scanner to Delete', 'turbo-guard-security-malware-scanner' ); ?>
				</a>
			</p>
			<table class="widefat">
				<thead>
					<tr>
						...
						<th style="width:130px;"><?php esc_html_e( 'Modified', 'turbo-guard-security-malware-scanner' ); ?></th>
						<th><?php esc_html_e( 'Why Suspicious', 'turbo-guard-security-malware-scanner' ); ?></th>
					</tr>
				</thead>
				<tbody>
		...
		<!-- Google check -->
		<div class="turbo-guard-card">
			<h2><?php esc_html_e( 'Check What Google Has Indexed', 'turbo-guard-security-malware-scanner' ); ?></h2>
			<p style="font-size:13px;color:#6b7280;line-height:1.6;">
				<?php esc_html_e( 'After cleaning files and spam posts, check if Google has already indexed any spam pages. Look for any pages with Japanese, Chinese, or spam product titles.', 'turbo-guard-security-malware-scanner' ); ?>
			</p>
			<div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:12px;">
				<a href="<?php echo esc_url( $tg_seo_results['google_link'] ); ?>" target="_blank" rel="noopener noreferrer" class="button button-primary">
					...
					printf(
						/* translators: %s: domain name */
						esc_html__( 'View site:%s on Google', 'turbo-guard-security-malware-scanner' ),
						esc_html( $tg_domain )
					);
					?>
				</a>
				<a href="<?php echo esc_url( admin_url( 'admin.php?page=turbo-guard-gsc' ) ); ?>" class="button">
					<?php esc_html_e( 'Connect GSC to Remove from Google', 'turbo-guard-security-malware-scanner' ); ?>
				</a>
			</div>
		</div>
	...
	<?php else : ?>
		<div class="turbo-guard-card">
			<div style="text-align:center;padding:40px 20px;color:#6b7280;">
				...
				<strong style="font-size:15px;"><?php esc_html_e( 'No scan run yet.', 'turbo-guard-security-malware-scanner' ); ?></strong>
				<p style="margin-top:6px;"><?php esc_html_e( 'Click "Scan for SEO Spam" above to check your site instantly.', 'turbo-guard-security-malware-scanner' ); ?></p>
			</div>
		</div>
	<?php endif; ?>
...
jQuery(document).ready(function($) {
	...
	$(document).on('click', '.turbo-guard-delete-spam-post', function() {
		if (!confirm('<?php esc_html_e( 'Permanently delete this spam post?', 'turbo-guard-security-malware-scanner' ); ?>')) return;
		var $btn = $(this).prop('disabled', true).text('<?php esc_html_e( 'Deleting...', 'turbo-guard-security-malware-scanner' ); ?>');
		...
		$.post(turboGuardAdmin.ajaxUrl, { action: 'turbo_guard_delete_spam_post', nonce: nonce, post_id: id }, function(r) {
			...
			else { $btn.prop('disabled', false).text('<?php esc_html_e( 'Delete (Free)', 'turbo-guard-security-malware-scanner' ); ?>'); }
		});
	});
	$('#turbo-guard-delete-all-spam-posts').on('click', function() {
		var ids = $(this).data('ids').toString().split(',');
		if (!confirm('<?php esc_html_e( 'Delete all spam posts? This cannot be undone.', 'turbo-guard-security-malware-scanner' ); ?>')) return;
		var $btn = $(this).prop('disabled', true);
		var done = 0;
