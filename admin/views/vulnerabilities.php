...
<div class="wrap turbo-guard-vulnerabilities">
	...
	<div class="turbo-guard-page-header">
		<div class="turbo-guard-page-header-left">
			...
			<div>
				<h1><?php esc_html_e( 'Vulnerability Scanner', 'turbo-guard-security-malware-scanner' ); ?></h1>
				<p><?php esc_html_e( 'Check plugins, themes &amp; WordPress core for known CVEs', 'turbo-guard-security-malware-scanner' ); ?></p>
			</div>
		</div>
		<?php if ( $results ) : ?>
	...
	<div class="turbo-guard-card">
		<div class="turbo-guard-scan-hero">
			<div class="turbo-guard-scan-intro">
				<h2><?php esc_html_e( 'Vulnerability Check', 'turbo-guard-security-malware-scanner' ); ?></h2>
				<p>
					<?php esc_html_e( 'Checks all installed plugins and themes against the WPScan vulnerability database. Add a free WPScan API key in Settings for more detailed results.', 'turbo-guard-security-malware-scanner' ); ?>
				</p>
			</div>
			<button id="turbo-guard-run-vuln-scan" class="button button-primary button-large">
				<span class="dashicons dashicons-search"></span>
				<?php esc_html_e( 'Run Vulnerability Scan', 'turbo-guard-security-malware-scanner' ); ?>
			</button>
		</div>
		<div id="turbo-guard-vuln-scanning" style="display:none;margin-top:14px;padding-top:14px;border-top:1px solid #f3f4f6;">
			...
			<strong><?php esc_html_e( 'Scanning installed plugins and themes…', 'turbo-guard-security-malware-scanner' ); ?></strong>
		</div>
	...
	<?php if ( $results ) : ?>
		...
		<div class="turbo-guard-stats-grid" style="grid-template-columns:repeat(3,1fr);">
			<div class="turbo-guard-card turbo-guard-stat-card <?php echo $results['total'] > 0 ? 'tg-threat' : ''; ?>" style="padding:18px 20px;">
				<h3><?php esc_html_e( 'Total Vulnerabilities', 'turbo-guard-security-malware-scanner' ); ?></h3>
				<div class="turbo-guard-stat-value <?php echo $results['total'] > 0 ? 'tg-red' : 'tg-green'; ?>">
					<?php echo esc_html( $results['total'] ); ?>
				</div>
				<p class="turbo-guard-stat-label"><?php esc_html_e( 'Across plugins, themes &amp; core', 'turbo-guard-security-malware-scanner' ); ?></p>
			</div>
			<div class="turbo-guard-card" style="padding:18px 20px;">
				<h3><?php esc_html_e( 'Vulnerable Plugins', 'turbo-guard-security-malware-scanner' ); ?></h3>
				<div class="turbo-guard-stat-value <?php echo count( $results['plugins'] ) > 0 ? 'tg-red' : ''; ?>">
					<?php echo esc_html( count( $results['plugins'] ) ); ?>
				</div>
				<p class="turbo-guard-stat-label"><?php esc_html_e( 'Plugins with known CVEs', 'turbo-guard-security-malware-scanner' ); ?></p>
			</div>
			<div class="turbo-guard-card" style="padding:18px 20px;">
				<h3><?php esc_html_e( 'Vulnerable Themes', 'turbo-guard-security-malware-scanner' ); ?></h3>
				<div class="turbo-guard-stat-value <?php echo count( $results['themes'] ) > 0 ? 'tg-red' : ''; ?>">
					<?php echo esc_html( count( $results['themes'] ) ); ?>
				</div>
				<p class="turbo-guard-stat-label"><?php esc_html_e( 'Themes with known CVEs', 'turbo-guard-security-malware-scanner' ); ?></p>
			</div>
		</div>
		...
		<?php if ( ! empty( $results['wordpress'] ) ) : ?>
			...
			<div class="turbo-guard-card" style="border-left:4px solid #dc2626;">
				<h2 style="color:#dc2626;">
					...
					<?php esc_html_e( 'WordPress Core Vulnerabilities', 'turbo-guard-security-malware-scanner' ); ?>
				</h2>
				<p class="description"><?php esc_html_e( 'Update WordPress immediately to resolve these issues.', 'turbo-guard-security-malware-scanner' ); ?></p>
		...
		<?php if ( ! empty( $results['plugins'] ) ) : ?>
			<div class="turbo-guard-card">
				<h2><?php esc_html_e( 'Plugin Vulnerabilities', 'turbo-guard-security-malware-scanner' ); ?></h2>
					...
					<div class="turbo-guard-vuln-item">
						<div class="turbo-guard-vuln-plugin-header">
							...
							<span class="turbo-guard-badge turbo-guard-badge-critical" style="margin-left:8px;">
								...
								printf(
									/* translators: %d: count */
									esc_html( _n( '%d issue', '%d issues', $plugin['count'], 'turbo-guard-security-malware-scanner' ) ),
		...
		<?php if ( ! empty( $results['themes'] ) ) : ?>
			<div class="turbo-guard-card">
				<h2><?php esc_html_e( 'Theme Vulnerabilities', 'turbo-guard-security-malware-scanner' ); ?></h2>
					...
					<div class="turbo-guard-vuln-item">
						<div class="turbo-guard-vuln-plugin-header">
							...
							<span class="turbo-guard-badge turbo-guard-badge-high" style="margin-left:8px;">
								...
								printf(
									/* translators: %d: count */
									esc_html( _n( '%d issue', '%d issues', $theme['count'], 'turbo-guard-security-malware-scanner' ) ),
		...
		<?php if ( 0 === $results['total'] ) : ?>
			<div class="turbo-guard-card">
				<div class="turbo-guard-no-threats">
					...
					<p><?php esc_html_e( 'No known vulnerabilities found — all plugins and themes are clean!', 'turbo-guard-security-malware-scanner' ); ?></p>
				</div>
			</div>
		<?php endif; ?>
	...
	<?php else : ?>
		...
		<div class="turbo-guard-card">
			<div class="turbo-guard-no-scan">
				...
				<p><?php esc_html_e( 'No vulnerability scan has been run yet. Click "Run Vulnerability Scan" to start.', 'turbo-guard-security-malware-scanner' ); ?></p>
			</div>
		</div>
...
function turbo_guard_render_vuln_table( $vulns ) {
	...
	<table class="widefat turbo-guard-results-table" style="margin-top:10px;">
		<thead>
			<tr>
				<th><?php esc_html_e( 'Vulnerability', 'turbo-guard-security-malware-scanner' ); ?></th>
				<th style="width:90px;"><?php esc_html_e( 'Type', 'turbo-guard-security-malware-scanner' ); ?></th>
				<th style="width:90px;"><?php esc_html_e( 'Severity', 'turbo-guard-security-malware-scanner' ); ?></th>
				<th style="width:110px;"><?php esc_html_e( 'Fixed In', 'turbo-guard-security-malware-scanner' ); ?></th>
				<th style="width:60px;"><?php esc_html_e( 'CVE', 'turbo-guard-security-malware-scanner' ); ?></th>
			</tr>
		</thead>
		<tbody>
					...
					<td>
						...
						<?php if ( $v['url'] ) : ?>
							<br><a href="<?php echo esc_url( $v['url'] ); ?>" target="_blank" rel="noopener noreferrer" style="font-size:11px;">
								<?php esc_html_e( 'Details →', 'turbo-guard-security-malware-scanner' ); ?>
							</a>
						<?php endif; ?>
					</td>
