...
$turbo_guard_ir              = $integrity_results;
$turbo_guard_baseline        = get_option( 'turbo_guard_file_baseline', array() );
$turbo_guard_baseline_count  = count( $turbo_guard_baseline );
$turbo_guard_watcher_last    = isset( $watcher_last_run ) ? $watcher_last_run : '';
$turbo_guard_baseline_at     = isset( $baseline_built_at ) ? $baseline_built_at : '';
?>
...
<div class="wrap turbo-guard-integrity">
	...
	<?php if ( $turbo_guard_ir && ! empty( $turbo_guard_ir['results'] ) ) : ?>
	<div class="turbo-guard-card">
		...
		<table class="widefat">
			...
			<tbody>
				<?php foreach ( $turbo_guard_ir['results'] as $turbo_guard_ir_row ) : ?>
				<tr>
					<td><code><?php echo esc_html( $turbo_guard_ir_row['path'] ); ?></code></td>
					<td>
						<span class="turbo-guard-badge turbo-guard-badge-critical">
							<?php echo esc_html( ucfirst( $turbo_guard_ir_row['status'] ) ); ?>
						</span>
					</td>
					<td style="font-size:12px;color:#6b7280;"><?php echo esc_html( $turbo_guard_ir_row['detail'] ); ?></td>
				</tr>
				<?php endforeach; ?>
			</tbody>
		</table>
